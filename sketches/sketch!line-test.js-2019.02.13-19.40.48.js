...
let mouseDistance = 0;
const sizeDivisor = 11;
let pointTotal = 6;
const lineSpacing = 10;

let rotations = Array(total).fill(0);
...
canvasSketch(() => {
  ...
  return {
    render({ p5, time, width, height }) {
      ...
      scale = p5.lerp(0, 1, Math.max(0, Math.min(1, (time - 3) / 5)));
      ...
      for (let j = 0; j < total; j++) {
        rotations[j] = p5.lerp(rotations[j], ((p5.noise(mouseDistance / dim, time / 2) - .5) / total) * Math.PI * 3, j / total);

        p5.rotate(rotations[j]);
        ...
        if (mousePos[1] > height / 2) {
          ...
          if (j % 4 === 0) {
            ...
            p5.stroke(150, 150, 160);
            p5.strokeWeight(2);
          } else {
            p5.stroke(240, 240, 245);
        ...
        for (let i = 0; i < pointTotal; i++) {
          let pos = [];
          let yNoise = 0;
          const size = (dim + (total - j) * lineSpacing * scale) / sizeDivisor;
          ...
          if (i <= 0 || i >= pointTotal - 1) {
            ...
            pos = [
              Math.sin((i / pointTotal) * Math.PI * 2) * size,
              Math.cos((i / pointTotal) * Math.PI * 2) * size
            ];
          }
